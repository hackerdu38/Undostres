...
def sens_jeu(main_joueur, tas_jeu, nb_joueurs, actif):
    ...
    if texte(tas_jeu[-1])=="changement sens":
       sens=-sens
        ...
        numero_jou=2
    actif = (actif+sens*numero_jou)%nb_joueurs
    return actif
...
def plus4(tas_jeu):
    i=-1
    compteurpioche=0
    while texte(tas_jeu[i])== "+2" or texte(tas_jeu[i])=="+4" and tas_jeu[-1]!=tas_jeu[0]:
        if texte(tas_jeu[i])=="+2":
...
def joue_ou_pioche(main_joueur, tas_jeu):
    global paquet, nb_cartes, mains, ordre_passage
    """
